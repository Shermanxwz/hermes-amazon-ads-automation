# Hermes Amazon Ads Control v4.2.2 — Sealed Full-Managed Release

v4.2.2 is the repository-sealed repair and autonomy release for the Hermes Amazon Ads control plane.
...
The `v4.2.2` Tag/Release is created only after the repository's `CI` workflow succeeds on the current `main` HEAD. The release workflow re-verifies:

- `pyproject.toml == 4.2.2`;
- `package-manifest.json == 4.2.2`;
- Hermes plugin version `== 4.2.2`;
- control-plane `__version__ == 4.2.2`;
- repository privacy/secret scan;
- current Hermes Studio integration contract.

If a `v4.2.2` tag already exists at a different commit, the workflow fails closed instead of moving the tag.
