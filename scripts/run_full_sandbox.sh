...
: > "$RESULTS"
record(){ local status="$1" layer="$2" name="$3" detail="${4:-}"; printf '%s\t%s\t%s\t%s\n' "$status" "$layer" "$name" "${detail//$'\n'/ }" >> "$RESULTS"; }
run_required(){ local layer="$1" name="$2"; shift 2; echo "==> $name"; if "$@"; then record PASS "$layer" "$name"; else local code=$?; record FAIL "$layer" "$name" "exit=$code"; fi; return 0; }
...
if python3 -c 'import playwright' >/dev/null 2>&1; then run_required browser "Real Chromium full-managed Web E2E" python3 "$ROOT/tests/browser_e2e.py"; run_required browser "Chromium/Firefox/WebKit matrix E2E" python3 "$ROOT/tests/browser_matrix_e2e.py"; else record EXTERNAL browser "Browser E2E" "playwright is not installed"; fi
...
record EXTERNAL amazon_live "Authenticated live MCP initialize and complete tools/list" "requires owner access token"
record EXTERNAL amazon_live "OAuth authorization and refresh rotation" "requires owner Login with Amazon consent"
record EXTERNAL amazon_live "Real Profiles, reports, 429 and Marketing Stream" "account-specific evidence"
record EXTERNAL amazon_live "Test Account full-managed Campaign hierarchy canary" "requires Test Account or bounded real profile"
record EXTERNAL amazon_live "Independent Amazon read-back and attribution-window evaluation" "requires matured advertiser data"
record EXTERNAL host_live "2C2G soak, reboot, HTTPS and backup-restore" "requires target VPS"
python3 - "$RESULTS" "$ARTIFACT_DIR/report.json" "$ARTIFACT_DIR/report.md" <<'PY'
import csv,json,sys
from collections import Counter,defaultdict
from datetime import datetime,timezone
from pathlib import Path
source,json_path,md_path=map(Path,sys.argv[1:]); rows=[]
with source.open(encoding="utf-8") as handle:
    for status,layer,name,detail in csv.reader(handle,delimiter="\t"): rows.append({"status":status,"layer":layer,"name":name,"detail":detail})
...
report={"generated_at":datetime.now(timezone.utc).isoformat(timespec="seconds"),"overall":overall,"counts":dict(counts),"layers":{k:dict(v) for k,v in sorted(layers.items())},"results":rows}
json_path.write_text(json.dumps(report,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
lines=["# Hermes Amazon Ads Full-Managed Sealed ACOS v5 Sandbox Report","",f"- Overall: **{overall}**",f"- PASS: {counts['PASS']}",f"- FAIL: {counts['FAIL']}",f"- EXTERNAL: {counts['EXTERNAL']}","","| Status | Layer | Check | Detail |","|---|---|---|---|"]
for row in rows: lines.append(f"| {row['status']} | {row['layer']} | {row['name']} | {row['detail'].replace('|',chr(92)+'|')} |")
md_path.write_text("\n".join(lines)+"\n",encoding="utf-8"); print(json.dumps({"overall":overall,**counts,"report":str(json_path)},ensure_ascii=False)); sys.exit(1 if overall=="FAIL" else 0)
PY
