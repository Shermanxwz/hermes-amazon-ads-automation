...
- Obtain a fresh Amazon Campaign read before any exposure-increasing action so the controller can prove the account daily budget envelope.
...
- Historical performance evidence controls action size and confidence; lack of mature historical performance data alone is not a reason to forbid a small exploration. When useful, create a bounded HERMES-SP-EXP-* exploratory Sponsored Products plan inside the controller-reported exploration pool.
...
- On any trust, budget, schema, OAuth, data-quality, storage or verification uncertainty, fail closed and leave exposure unchanged or lower.
...
def _hermes_command() -> list[str]:
    ...
    command += ["--source", "tool", "--max-turns", os.getenv("ADS_ORCHESTRATOR_MAX_TURNS", "90"), "-z", PROMPT]
    return command
