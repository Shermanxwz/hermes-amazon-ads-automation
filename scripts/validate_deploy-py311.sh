...
cd "$TMP/package-src"
if python3.11 -c 'import setuptools.build_meta' >/dev/null 2>&1; then
  python3.11 -m pip wheel . --no-deps --no-build-isolation -w "$TMP/dist" >/dev/null
else
  python3.11 -m pip wheel . --no-deps -w "$TMP/dist" >/dev/null
fi
python3.11 -m venv "$TMP/installed"
...
export ADS_CONTROL_AGENT_TOKEN="$(python3.11 -c 'print("x"*48)')"
...
export ADS_CONTROL_PASSWORD_HASH="$(PYTHONPATH="$ROOT/control-plane" python3.11 -c 'from amazon_ads_control.security import hash_password; print(hash_password("correct horse battery staple"))')"
...
PYTHONPATH="$INSTALLED_SITE" python3.11 "$ROOT/scripts/control_cli.py" storage-status --database "$TMP/state.db" >/dev/null
