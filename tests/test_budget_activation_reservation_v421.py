...
class BudgetActivationReservationV421Tests(unittest.TestCase):
    def setUp(self):
    ...
    def test_staged_enable_does_not_double_reserve_the_create_budget(self):
        create = {
            ...
            "payload": {"approved_args": {"campaigns": [{
                "name": "HERMES-SP-new", "budget": 30, "state": "PAUSED", "adProduct": "SPONSORED_PRODUCTS",
            }]}},
        }
        enable = {
        ...
        cycle = self.env.store.create_cycle(
            ...
            source="activation-reservation-test",
            window={"start": "2026-08-01", "end": "2026-08-01", "grain": "daily"},
        ...
        with self.env.store.connection() as conn:
            conn.execute(
                ...
                ((now - timedelta(seconds=2)).isoformat(), (now - timedelta(seconds=2)).isoformat(), rows["new"]["id"]),
            )
        ...
        self.assertEqual(before["projected_exposure"], 60)
        self.assertEqual(before["remaining"], 10)

        # If ENABLE were counted as another independent +30 Campaign budget,
        # this reservation would project 120 maximum-spend exposure and fail.
