...
class ConfigTests(unittest.TestCase):
    def test_strict_bool_and_int(self):
        with patch.dict(os.environ, {"X_BOOL": "yes", "X_INT": "42"}, clear=False):
            self.assertTrue(_bool("X_BOOL", False))
            self.assertEqual(_int("X_INT", 1, 1, 100), 42)
    ...
    def test_environment_validation(self):
        ...
        with patch.dict(os.environ, base, clear=True):
            ...
            self.assertEqual(settings.public_origin, "https://ads.example.com")
        ...
        with patch.dict(os.environ, {**base, "ADS_CONTROL_AGENT_TOKEN": " short "}, clear=True):
            with self.assertRaisesRegex(ValueError, "AGENT_TOKEN"):
    ...
    def test_server_help_version_and_check(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch.dict(os.environ, env, clear=True), redirect_stdout(output):
                self.assertEqual(server_module.main(["--check"]), 0)
...
class ControlCliTests(unittest.TestCase):
    def test_generate_token_and_password_errors(self):
        ...
        with redirect_stdout(output):
            self.assertEqual(control_cli.main(["generate-token"]), 0)
        self.assertGreaterEqual(len(output.getvalue().strip()), 48)
        with patch("getpass.getpass", side_effect=["a" * 14, "b" * 14]), redirect_stderr(io.StringIO()):
            self.assertEqual(control_cli.main(["hash-password"]), 2)
        with patch("getpass.getpass", side_effect=["short", "short"]), redirect_stderr(io.StringIO()):
            self.assertEqual(control_cli.main(["hash-password"]), 2)
    ...
    def test_verify_and_backup_database(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            Store(db).event("info", "test", "test", None, "hello", {})
            out = io.StringIO()
            with redirect_stdout(out):
                self.assertEqual(control_cli.main(["verify-database", "--database", str(db), "--full"]), 0)
            self.assertTrue(json.loads(out.getvalue())["ok"])
            out = io.StringIO()
            with redirect_stdout(out):
                self.assertEqual(control_cli.main(["backup", "--database", str(db), "--output", str(backup)]), 0)
            result = json.loads(out.getvalue())
            self.assertEqual(Path(result["path"]), backup)
            self.assertEqual(backup.stat().st_mode & 0o777, 0o600)
            self.assertEqual(len(Store(backup).list_events()), 1)
...
class ClientTests(unittest.TestCase):
    ...
    def test_control_client_response_handling(self):
        ...
        self.assertEqual(client.request("GET", "/list")["error"], "invalid_control_response")
        self.assertEqual(client.request("GET", "/invalid")["error"], "invalid_control_response")
        self.assertEqual(client.request("GET", "/error-json")["http_status"], 400)
        self.assertEqual(client.request("GET", "/error-text")["http_status"], 502)

    def test_control_client_unavailable(self):
        result = ControlClient("http://127.0.0.1:1", "x" * 48, timeout=0.1).request("GET", "/")
        self.assertEqual(result["error"], "control_plane_unavailable")
...
class WorkerCliTests(unittest.TestCase):
    ...
    def test_once_propagates_heartbeat_failure(self):
        from amazon_ads_control import worker
        with patch.dict(os.environ, {"ADS_CONTROL_AGENT_TOKEN":"x"*48}, clear=True), patch.object(worker.ControlClient,"request",return_value={"error":"down"}), redirect_stderr(io.StringIO()):
            self.assertEqual(worker.main(["--once"]), 2)
