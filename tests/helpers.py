...
class Environment:
    def __init__(self, *, strict_writes: bool = False):
        ...
        # A few historical tests passed a human-readable fixture actor as a
        # second positional argument. Keep that compatibility strictly inside
        # the test Environment; the production Store API remains exact.
        original_ingest_stream_events = self.store.ingest_stream_events

        def ingest_stream_events_fixture(events, _actor=None):
            return original_ingest_stream_events(events)

        self.store.ingest_stream_events = ingest_stream_events_fixture

        original_upsert_profile = self.store.upsert_profile

        def upsert_profile_with_spend_fixture(profile):
            result = original_upsert_profile(profile)
            profile_id = str(profile.get("profile_id") or "").strip()
            if profile_id:
                now = datetime.now(UTC).replace(microsecond=0)
                date = now.astimezone(ZoneInfo("America/Los_Angeles")).date().isoformat()
                self.store.ingest_stream_events([{
                    "profile_id": profile_id,
                    "dataset_id": "sp-traffic",
                    "event_time": now.isoformat(),
                    "dedupe_key": f"test-zero-spend:{profile_id}:{date}",
                    "payload": {
                        "date": date,
                        "adProduct": "SPONSORED_PRODUCTS",
                        "campaignId": "test-zero",
                        "cost": 0,
                    },
                }], "test-fixture")
            return result

        self.store.upsert_profile = upsert_profile_with_spend_fixture
