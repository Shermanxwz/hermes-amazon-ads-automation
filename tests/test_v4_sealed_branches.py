...
from amazon_ads_control.probabilistic_acos import (
    DelayModel, PosteriorConfig, account_aov, cumulative_delay_from_rows,
    estimate_acos_posterior,
)
from amazon_ads_control.sealed_envelope import (
    envelope_hash, marker_shape_valid, policy_for, standing_authorized,
)
...
from amazon_ads_control.transport_attestation import (
    REQUIRED_SP_OPERATIONS, attest_profile_capabilities, validate_attestation,
)
...
def campaign_args(name="HERMES-SP-P1-EXACT-001", budget=20, state="PAUSED", product="SPONSORED_PRODUCTS"):
...
class ProbabilisticBranchTests(unittest.TestCase):
    def test_delay_curve_normalization_interpolation_and_learning(self):
        ...
        fitted = cumulative_delay_from_rows([
    ...
    def test_aov_and_posterior_edge_paths(self):
        ...
        invalid = estimate_acos_posterior(
            ...
            target_acos="bad", max_acos="bad", age_days=None,
        )
        self.assertIsNone(invalid.expected_acos)
        self.assertEqual((invalid.p_acos_over_target, invalid.p_acos_under_target), (0, 1))

    def test_policy_clamps_delay_flags_and_aggregate_threshold(self):
        policy = StrategyPolicy.from_mapping({
            "delay_curve": "0.2,0.1,0.7", "posterior_budget_scale_probability": 2,
            "posterior_min_confidence": -1, "enable_hourly_pacing": "off",
            "sealed_sp_namespace": "X" * 80, "sealed_sp_max_campaign_creates_per_day": 0,
        })
        self.assertEqual(policy.delay_curve, (Decimal("0.2"), Decimal("0.2"), Decimal("0.7"), Decimal("1")))
        self.assertEqual(policy.posterior_budget_scale_probability, Decimal("0.95"))
        self.assertEqual(policy.posterior_min_confidence, 0)
        self.assertFalse(policy.enable_hourly_pacing)
        self.assertEqual((len(policy.sealed_sp_namespace), policy.sealed_sp_max_campaign_creates_per_day), (40, 2))
...
class ControllerBranchTests(unittest.TestCase):
    def test_exact_placement_evidence_wins_over_campaign_aggregate(self):
        ...
        plan = OptimizationEngine()._placement(
            "p1", "c1", "PLACEMENT_TOP", Decimal("0"), Decimal("10"), Decimal("10"),
            "ADS-PLACEMENT-TOS-SCALE", 60, "test", {"placement": "PLACEMENT_TOP"},
        )
        self.assertEqual(decision_row(plan, row_index(snapshot))["sales"], 100)
    ...
    def test_lifecycle_quarantine_and_verified_recovery(self):
        ...
        self.assertEqual((quarantined[0].rule_id, quarantined[0].payload["after"]), ("ADS-LIFECYCLE-QUARANTINE", "PAUSED"))
        good = one_target_snapshot()
        good["targets"][0].update({"state": "PAUSED", "recovery_ready": True, "clicks": 180, "orders": 30, "sales": 1500, "spend": 180})
    ...
    def test_global_budget_transfer_and_no_cap_path(self):
        ...
        snapshot["budget_usage"] = [{"campaign_id": "winner", "budget_usage_percent": 95}, {"campaign_id": "loser", "budget_usage_percent": 80}]
    ...
    def test_hourly_pacing_emits_bounded_up_and_down(self):
        policy = StrategyPolicy.from_mapping({"posterior_reduce_probability": 0.7, "posterior_scale_probability": 0.7})
        snapshot = one_target_snapshot()
        snapshot["account"].update({"sales": 500, "orders": 10})
        snapshot["targets"] = [
            {"target_id": "down", "campaign_id": "bad", "state": "ENABLED", "bid": 1},
            {"target_id": "up", "campaign_id": "good", "state": "ENABLED", "bid": 1},
        ]
        snapshot["hourly"] = [
...
class SealedEnvelopeBranchTests(unittest.TestCase):
    def setUp(self):
        ...
        descriptor = descriptor_from_payload(CREATE_CAMPAIGN)
        self.tool = descriptor.as_dict()
        self.env.store.sync_catalog([descriptor])
        policy = policy_for(self.env.store, "p1")
        self.marker = {"version": 1, "validated": True, "scope": "sealed-sp", "profile_id": "p1",
            "ad_product": "SPONSORED_PRODUCTS", "observed_in_ads": True, "purpose": "structural_maintenance",
            "desired_state": "PAUSED", "envelope_hash": envelope_hash("p1", policy)}

    def tearDown(self):
        self.env.close()

    def decision(self, action="create_campaign", args=None, marker=None, family="campaign"):
        return {"profile_id": "p1", "action_type": action, "expected_family": family, "payload": {
            "approved_args": args or campaign_args(), "ad_product": "SPONSORED_PRODUCTS",
    ...
    def test_valid_marker_and_core_denials(self):
        ...
        changed = self.decision(); changed["payload"]["standing_authorization"]["envelope_hash"] = "stale"
        self.assertIn("scope changed", standing_authorized(self.env.store, changed, self.tool)[1])
        self.assertIn("Sponsored Products only", standing_authorized(self.env.store, self.decision(args=campaign_args(product="SPONSORED_BRANDS")), self.tool)[1])
        destructive = self.decision(action="delete_campaign")
        self.assertIn("permanently blocked", standing_authorized(self.env.store, destructive, {**self.tool, "native_name": "campaign-delete"})[1])
    ...
    def test_state_product_ad_and_disabled_denials(self):
        ...
        ad = self.decision(action="create_ad", args={"state": "PAUSED", "adProduct": "SPONSORED_PRODUCTS"}, family="ad")
    ...
    def test_plan_validator_shape_schema_and_exposure_denials(self):
        base = {"profile": {"profile_id": "p1"}, "actions": [{
            "tool_name": CREATE_CAMPAIGN["registered_name"], "action_type": "create_campaign",
        ...
        for actions, message in (([], "1-50"), (["bad"], "object"), ([{"tool_name": "missing", "arguments": {}}], "live catalog")):
            with self.assertRaisesRegex(ValueError, message):
                validate_standing_plan(self.env.service, {**base, "actions": actions})
        invalid = deepcopy(base); invalid["actions"][0]["arguments"] = {"campaigns": []}
        with self.assertRaisesRegex(ValueError, "schema"):
            validate_standing_plan(self.env.service, invalid)
        too_many = deepcopy(base); too_many["actions"] = [deepcopy(base["actions"][0]) for _ in range(3)]
        ...
        over = deepcopy(base); over["actions"] = [deepcopy(base["actions"][0]) for _ in range(2)]
        with self.assertRaisesRegex(ValueError, "budget envelope"):
            validate_standing_plan(self.env.service, over)
...
class AttestationAndUnifiedBranchTests(unittest.TestCase):
    def test_attestation_mcp_only_missing_and_validation_errors(self):
        tools = [
            ...
            {"name": "campaign-delete", "semantic": "write"}, "ignored",
        ]
        attestation = attest_profile_capabilities(profile_id="p1", region="NA", tools=tools, direct_api_operations=[])
        ...
        self.assertEqual((campaign.primary, campaign.fallback), ("campaign-create", None))
    ...
    def test_unified_contract_error_and_local_main_paths(self):
        malformed = {"item": ["ignored", {"name": "Folder", "item": [{"name": "Bad body", "request": {
            "method": "POST", "url": "https://example.com/not-unified", "body": {"raw": "{"}}}]}]}
        ...
        with tempfile.TemporaryDirectory() as td:
            source, output = Path(td) / "unified.json", Path(td) / "manifest.json"
